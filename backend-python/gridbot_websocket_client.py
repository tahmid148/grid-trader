...
for i in range(config.NUM_BUY_GRID_LINES):
    ...
    order_price = initial_price - ((i + 1) * config.GRID_SIZE) # Order should be the below and dependant on size of the grid
...
for i in range(config.NUM_SELL_GRID_LINES):
    initial_price = ticker['bid']
    order_price = initial_price + ((i + 1) * config.GRID_SIZE) # Order should be the above and dependant on size of the grid
...
while True:
    ...
    for closed_order_id in closed_order_ids:
        buy_orders = [buy_order for buy_order in buy_orders if buy_order['orderId'] != closed_order_id]
        sell_orders = [sell_order for sell_order in sell_orders if sell_order['orderId'] != closed_order_id]
