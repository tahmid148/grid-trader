...
def CHOP(df, chop_len, atr_len):
    ...
    chop_series = ta.chop(high=df['High'], low=df['Low'],
                          close=df['Close'], length=chop_len, atr_length=atr_len)
    return chop_series
...
def plot_chart(i, symbol, df, current_price, buy_grid, sell_grid, buy_stop_loss, sell_stop_loss):
    ...
    #  Plot close price
    fig.add_trace(
        go.Scatter(x=df.index, y=df['Close'], line=dict(
            color="blue", width=1), name=f"Close"),
        row=1, col=1)
    #  Current price
    fig.add_hline(y=current_price, line_width=0.6,
                  line_dash="solid", line_color="blue", row=1, col=1)
    #  Add buy and sell grids
    i = 0
    for level in buy_grid:
        line_color = buy_grid_colors[i] if i < len(
            buy_grid_colors) else buy_grid_colors[0]
        fig.add_hline(y=level, line_width=0.6, line_dash="dash",
                      line_color=line_color, row=1, col=1)
        i += 1
    #  stop loss
    fig.add_hline(y=buy_stop_loss, line_width=0.6,
                  line_dash="solid", line_color="red", row=1, col=1)
    i = 0
    for level in sell_grid:
        line_color = sell_grid_colors[i] if i < len(
            sell_grid_colors) else sell_grid_colors[0]
        fig.add_hline(y=level, line_width=1, line_dash="dash",
                      line_color=line_color, row=1, col=1)
        i += 1
    #  stop loss
    fig.add_hline(y=sell_stop_loss, line_width=1,
                  line_dash="solid", line_color="red", row=1, col=1)
    fig.update_layout(
        title={'text': '', 'x': 0.5},
    ...
    def next(self):
        ...
        if not self.grid_in_progress and is_ranging:
            ...
            for i in range(1, self.num_grid_lines + 1):
                ...
                #  Create buy order
                self.buy(size=0.1, limit=grid_buy_price,
                         sl=buy_stop_loss, tp=buy_take_profit)
...
def run_backtest(df):
    ...
    bt = Backtest(df, GridStrategy, cash=10000, commission=.00075,
                  trade_on_close=True, exclusive_orders=False, hedging=False)
    stats = bt.run()
    print(stats)
    bt.plot()
